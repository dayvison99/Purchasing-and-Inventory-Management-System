...
<body id="page-top">
    ...
    <?php
      ...
      <?php
          $jn = "SELECT * FROM cvs3 as b INNER JOIN cvs as j ON b.codproduto = j.codproduto where b.estaual > b.estminino order by b.descproduto";
          $jnresult = mysqli_query($con, $jn);

          $bm = "SELECT * FROM cvs3 where empresa = 5 and estaual > estminino order by descproduto ";
          $bmresult = mysqli_query($con, $bm);

          $sf = "SELECT * FROM cvs3 as b INNER JOIN cvs2 as s ON s.codproduto = b.codproduto where b.estaual > b.estminino order by b.descproduto";
      ...
      <?php
               while($rowjn = mysqli_fetch_assoc($jnresult)) {
                  ...
                  #Cabecario BM
                  echo "<tr >";
                  echo "<td>" .$rowbm['codproduto'];"</td>";
                  echo "<td>" .$rowbm['descproduto'];"</td>";

                  #se estoque de atual de BM for maior que o estoque minino
                  if ($rowbm['estaual'] > $rowbm['estminino']){
                    echo "<td>" .$difbm;"</td>";
                  }
                  else{
                    echo "<td>-</td>";
                  ...
                  if($difbm > 0){
                        ...
                        #Jn precinsando de BM
                            if ($rowjn['estaual'] < $rowjn['estminino'] and $rowsf['estaual'] < $rowsf['estminino'] ){
                              #BM tem mais que jn precisa
                                if($rowbm['estaual'] - $rowbm['estminino'] >= $rowjn['estminino']-$rowjn['estaual'] and $difsf < 0){
                                  echo "<td>".($rowjn['estminino']-$rowjn['estaual']);"</td>";
                                }
                              #BM tem menos que jn precisa
                                if($rowbm['estaual'] - $rowbm['estminino'] < $rowjn['estminino']-$rowjn['estaual'] and $difsf < 0){
                                  echo "<td>".($rowbm['estaual'] - $rowbm['estminino']);"</td>";
                                }
                            ...
                            if ($rowjn['estaual'] < $rowjn['estminino'] and $rowsf['estaual'] >= $rowsf['estminino'] ){
                                ...
                                if($rowjn['estminino']-$rowjn['estaual'] > $rowsf['estaual'] - $rowsf['estminino'] and $difbm >= $rowjn['estminino']-$rowjn['estaual']-($rowsf['estaual'] - $rowsf['estminino'])  ){
                                    echo "<td>".($rowjn['estminino']-$rowjn['estaual']-($rowsf['estaual'] - $rowsf['estminino']));"</td>";
                    ...
                    if ($rowsf['estaual'] < $rowsf['estminino'] and $rowjn['estaual'] <= $rowjn['estminino'] ){
                      #BM tem mais que jn precisa
                        if($rowbm['estaual'] - $rowbm['estminino'] > $rowsf['estminino']-$rowsf['estaual'] and $difjn < 0){
                          echo "<td>a".($rowsf['estminino']-$rowsf['estaual']);"</td>";
                        }
                      #BM tem menos que jn precisa
                        if($rowbm['estaual'] - $rowbm['estminino'] < $rowsf['estminino']-$rowsf['estaual'] and $rowjn['estaual'] <= $rowjn['estminino'] ){
                                if(-$difjn < $rowbm['estaual'] - $rowbm['estminino']){
                                echo "<td>b".(-$difjn);"</td>";
                      ...
                      if ($rowsf['estaual'] < $rowsf['estminino'] and $rowjn['estaual'] > $rowjn['estminino'] ){
                        if($difjn < $rowsf['estminino'] - $rowsf['estaual']){
                            echo "<td>c".(($rowsf['estminino'] - $rowsf['estaual'])-$difjn);"</td>";
                          }
                            if($difjn > $rowsf['estminino'] - $rowsf['estaual']){
                                echo "<td>d-</td>";
