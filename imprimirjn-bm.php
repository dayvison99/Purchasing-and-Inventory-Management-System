      ...
      <div class="jumbotron" id="tabela"><center>
        <table id="tabela" class="table table-striped table-bordered table-condensed table-hover" title="Balanceamento!">
           <tr >
             ...
             <th><a class="btn btn-primary" href="sugestao.php">Voltar</a></th>
             <th colspan=""><a href="imprimirjn-bm-excel.php" id="jn"><i class="ion-help"></i> <button type="button" class="btn btn-primary">Gerar Excel</button></a></li></th>
             <th><a class="btn btn-primary" onclick="CriaPDF()" href="sugestao.php">Imprimir</a></th>
        ...
        <?php
                 while($rowjn = mysqli_fetch_assoc($lojasresult)) {
                  ...
                  if ($rowjn['jnestaual'] > $rowjn['jnestminino'] and $rowbm['bmestaual'] < $rowbm['bmestminino'] and $difjn > $rowsf['sfestminino'] - $rowsf['sfestaual']){
                    #cabecario jn
                    echo "<tr >";
                    echo "<td>" .$rowjn['jncodprduto'];"</td>";
                    echo "<td>" .$rowjn['jndescproduto'];"</td>";
                    echo "<td>" .$rowjn['jnestminino'];"</td>";


                    #se estoque de atual de jn for maior que o estoque minino
                  #  if ($rowjn['estaual'] > $rowjn['estminino']){
                      echo "<td>" .$difjn;"</td>";
                      ...
                      if ($rowbm['bmestaual'] < $rowbm['bmestminino'] and $rowjn['jnestaual'] > $rowjn['jnestminino']){
                        if ($rowbm['bmestaual'] < $rowbm['bmestminino'] and $rowsf['sfestaual'] >= $rowsf['sfestminino'] and $rowjn['jnestaual'] > $rowjn['jnestminino']){
                          if ($difjn >= $rowbm['bmestminino'] - $rowbm['bmestaual']){
                              echo "<td>" .($rowbm['bmestminino'] -$rowbm['bmestaual']);"</td>";
                              }
                          if ($difjn < $rowbm['bmestminino'] - $rowbm['bmestaual']){
                              echo "<td>" .($difjn);"<td>";
                              }
                        ...
                        if ($rowbm['bmestaual'] < $rowbm['bmestminino'] and $rowsf['sfestaual'] < $rowsf['sfestminino'] and $rowjn['jnestaual'] > $rowjn['jnestminino']){
                          ...
                          if ($difjn > $rowsf['sfestminino'] - $rowsf['sfestaual']){
                              ...
                              if ($difjn-($rowsf['sfestminino'] - $rowsf['sfestaual']) <$rowbm['bmestminino'] -$rowbm['bmestaual']){
                                    echo "<td>" .($difjn-($rowsf['sfestminino'] - $rowsf['sfestaual']));"</td>";
                                    }
                                  }
                                }
                  }
...
function CriaPDF() {
    ...
    win.document.write('<title>Balanceamento Januária para Brasília de Minas</title>');   // <title> CABEÇALHO DO PDF.
