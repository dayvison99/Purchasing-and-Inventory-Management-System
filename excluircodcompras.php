...
if ($_SESSION['empresa'] == "2") {
  ...
  while ($aux = mysqli_fetch_assoc($resultaux)) {
    $auxcod = $aux['codproduto'];
    $auxqtde = $aux['quantidade'] - 1;
    $alter = "UPDATE leitorcomprassf set quantidade = $auxqtde  where codproduto = $auxcod and idusuario = $idusuario ";
    $inserir = mysqli_query($con, $alter);
  }
}
if ($_SESSION['empresa'] == "5") {
  ...
  while ($aux = mysqli_fetch_assoc($resultaux)) {
    $auxcod = $aux['codproduto'];
    $auxqtde = $aux['quantidade'] - 1;
    $alter = "UPDATE leitorcomprasbm set quantidade = $auxqtde  where codproduto = $auxcod and idusuario = $idusuario ";
    $inserir = mysqli_query($con, $alter);
  }
}
if ($_SESSION['empresa'] == "6") {
  $resultaux = mysqli_query($con, "SELECT * FROM leitorcomprasmaimai where id = $excluir ");
  while ($aux = mysqli_fetch_assoc($resultaux)) {
    $auxcod = $aux['codproduto'];
    $auxqtde = $aux['quantidade'] - 1;
    $alter = "UPDATE leitorcomprasmaimai set quantidade = $auxqtde  where codproduto = $auxcod and idusuario = $idusuario ";
    $inserir = mysqli_query($con, $alter);
  }
}
...
if ($_SESSION['empresa'] == "6")
  $user = "DELETE FROM leitorcomprasmaimai WHERE id = $excluir";
