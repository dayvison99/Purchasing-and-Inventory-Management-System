...
<body>
  ...
  $arquivo = 'BALANCEAMENTO JANUARIA_SAOFRANCISCO.xls';

  // Criamos uma tabela HTML com o formato da planilha
  $html = '';
  $html .= '<table width="467" border="1px">';
  $html .= '<tr>';
  $html .= '<td colspan="5"><center>BALANCEAMENTO JANUÁRIA PARA SÃO FRANCISCO </center></tr>';
  $html .= '</tr>';
    ...
    if ($rowjn['jnestaual'] > $rowjn['jnestminino'] and $rowsf['sfestaual'] < $rowsf['sfestminino']){
      ...
      if ($rowsf['sfestaual'] < $rowsf['sfestminino'] and $rowjn['jnestaual'] > $rowjn['jnestminino']){
        if ($difjn >= $rowsf['sfestminino'] - $rowsf['sfestaual']){
          $html .= '<td>'.($rowsf['sfestminino'] -$rowsf['sfestaual']).'</td>';
