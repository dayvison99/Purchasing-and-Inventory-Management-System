  ...
  <input type="submit" class="btn btn-primary" value="Terminar"/>
    <a href="gerar_planilhacompras.php" id="jn"><i class="ion-help"></i> <button type="button" class="btn btn-primary">Imprimir Entrada</button></a></li>
      ...
      if ($_SESSION['empresa'] == "1"){
                  $sql = "SELECT * FROM leitorcomprasjn as l INNER JOIN est004 as e ON l.codproduto = e.CONTAPRODUTO where e.empresa = 1 and idusuario = $idusuario and quantidade > 0 order by l.id desc   ";
                  $result = mysqli_query($con, $sql);

                  $produtos = "SELECT * FROM est004 as e INNER JOIN leitorcomprasjn as l ON l.codproduto = e.CONTAPRODUTO where e.empresa = 1 and idusuario = $idusuario order by l.id desc  ";
                  $produtosresult = mysqli_query($con, $produtos);
                      }
      if ($_SESSION['empresa'] == "2"){
                $sql = "SELECT * FROM leitorcomprassf as l INNER JOIN est004 as e ON l.codproduto = e.CONTAPRODUTO where e.empresa = 1 order by l.id desc   ";
                $result = mysqli_query($con, $sql);

                $produtos = "SELECT * FROM est004 as e INNER JOIN leitorcomprassf as l ON l.codproduto = e.CONTAPRODUTO where e.empresa = 1 order by l.id desc  ";
                $produtosresult = mysqli_query($con, $produtos);
                      }
      if ($_SESSION['empresa'] == "5"){
                $sql = "SELECT * FROM leitorcomprasbm as l INNER JOIN est004 as e ON l.codproduto = e.CONTAPRODUTO where e.empresa = 1 order by l.id desc   ";
                $result = mysqli_query($con, $sql);

                $produtos = "SELECT * FROM est004 as e INNER JOIN leitorcomprasbm as l ON l.codproduto = e.CONTAPRODUTO where e.empresa = 1 order by l.id desc  ";
                $produtosresult = mysqli_query($con, $produtos);
                      }
      ?>
