...
<div class="jumbotron"><center>
  ...
  <legend class="legend-border"><h4> Estoque Atual Januária </h4></legend>
...
$leitor = "SELECT e.CODPRODUTO  as CONTAP,e.CONTAPRODUTO,e.DESCPRODUTO as descricao, l.CODPRODUTO as codigo, count(l.codproduto) as quantidade FROM est004 as e INNER JOIN leitorbm as l ON l.codproduto = e.CONTAPRODUTO where e.empresa = 1 group by l.codproduto order by descricao  ";
$leitorresult = mysqli_query($con, $leitor);

$sql = "SELECT e.estoque, e.codproduto  as CONTAP,e.descricao as descricao, l.id,l.codproduto as lcodigo,count(l.codproduto) as lqtde  FROM estatual as e INNER JOIN leitorbm as l ON l.codproduto = e.CODPRODUTO group by e.CODPRODUTO  UNION SELECT e.estoque,e.codproduto  as CONTAP,e.descricao as descricao, l.id,l.codproduto as lcodigo,count(l.codproduto) as lqtde FROM estatual as e INNER JOIN leitorbm as l ON l.codproduto = e.CODPRODUTO group by e.CODPRODUTO ";
...
//3º Reinicie o servidor http
    try{
        require_once 'lj/GetProdutosGrade.php';
        ...
        $result = "DELETE FROM `auxestoque`";
        $queryresult = mysqli_query($con, $result);
        $zerar = "ALTER TABLE `auxestoque` AUTO_INCREMENT=1";
        $queryresult = mysqli_query($con, $zerar);
        // Loop para percorrer o Objeto
        foreach($json->LJSISTEMAS as $arquivo):

          $empresa = $arquivo->EMPRESA;
          $codproduto = $arquivo->CODPRODUTO;
          $estoque = $arquivo->ESTOQUE;
          $descricao = $arquivo->DESCPRODUTO;


          $result = "INSERT INTO auxestoquejn(`empresa`,`codproduto`, `estoque`, `descricao`) VALUES ('$empresa','$codproduto','$estoque','$descricao')";
          $queryresult = mysqli_query($con, $result);
