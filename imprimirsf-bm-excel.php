 ...
 <link href="css/sb-admin-2.min.css" rel="stylesheet">
	...
	<body>
		...
		$arquivo = 'BALANCEAMENTO SÃOFRANCISCO_BRASILIA.xls';

		// Criamos uma tabela HTML com o formato da planilha
		$html = '';
		$html .= '<table width="467" border="1px">';
		$html .= '<tr>';
		$html .= '<td colspan="5"><center>BALANCEAMENTO SÃO FRANCISCO PARA BRASILIA</center></tr>';
		$html .= '</tr>';

		$html .= '<tr>';
    $html .= '<td><b>Codigo</b></td>';
		$html .= '<td><b>Descrição</b></td>';
		$html .= '<td><b>Minimo SF</b></td>';
		$html .= '<td><b>Excendentes SF</b></td>';
    ...
    while($rowjn = mysqli_fetch_assoc($lojasresult)) {
      ...
      if ($rowsf['sfestaual'] > $rowsf['sfestminino']  and $rowbm['bmestaual'] < $rowbm['bmestminino'] ){
         if($rowjn['jnestaual'] - $rowjn['jnestminino'] <  $rowbm['bmestminino'] - $rowbm['bmestaual'] and $rowjn['jnestminino'] - $rowjn['jnestaual'] < $rowsf['sfestaual'] - $rowsf['sfestminino']    ){
           ...
           #cabecario sf
           $html .= "<tr >";
           $html .= "<td>" .$rowsf['sfcodprduto'];"</td>";
           $html .= "<td>" .$rowsf['sfdescproduto'];"</td>";
           $html .= "<td>" .$rowsf['sfestminino'];"</td>";
         ...
         #CONDIÇÃO PARA SABER SE LOJA DE BM PRECISA DE SF
          if($difjn > 0 and $difbm < 0 and $difsf > 0){
            ...
            if ($difjn < $rowbm['bmestminino'] - $rowbm['bmestaual']){
                $html .= "<td>" .($rowbm['bmestminino'] -$rowbm['bmestaual']-$difjn);"</td>";
