...
<div class="form-group col-xs-6" >
      ...
      <?php
           ...
           $leitor = "SELECT e.CODPRODUTO  as CONTAP,e.DESCPRODUTO as descricao, l.CODPRODUTO as codigo, count(l.codproduto) as quantidade FROM auxestoquesf as e INNER JOIN leitorsf as l ON l.codproduto = e.CONTAPRODUTO where e.empresa = 1 group by l.codproduto order by descricao  ";
           $leitorresult = mysqli_query($con, $leitor);

           $sql = "SELECT e.estoque, e.codproduto  as CONTAP,e.descricao as descricao, l.id,l.codproduto as lcodigo,count(l.codproduto) as lqtde  FROM auxestoquesf as e INNER JOIN leitorsf as l ON l.codproduto = e.CODPRODUTO group by e.CODPRODUTO  UNION SELECT e.estoque,e.codproduto  as CONTAP,e.descricao as descricao, l.id,l.codproduto as lcodigo,count(l.codproduto) as lqtde FROM auxestoquesf as e INNER JOIN leitorsf as l ON l.codproduto = e.CODPRODUTO group by e.CODPRODUTO ";
           $resultsql = mysqli_query($con, $sql);


           $produtos = "SELECT  * FROM est004 where EMPRESA = 2";
        ...
        while($row = mysqli_fetch_assoc($resultsql)) {
            #  $leitor = mysqli_fetch_assoc($leitorresult);
             ...
             if($row['lcodigo'] == $row['CONTAP']){
              ...
              #if($row['lcodigo'] > 0 or $row['CONTAP'] >0) {
              #    if(($row['lcodigo'] != $row['CONTAP']) or ($row['lcodigo'] = $row['CONTAP'])) {
              #        echo "<tr >";
              #          echo "<td>" .$row['lcodigo'];"</td>";
              #          if($row['lcodigo'] > 0){
              #          echo "<td>" .$row['descricao'];"</td>";
