      ...
      <div class="jumbotron" id="tabela"><center>
        <table id="tabela" class="table table-striped table-bordered table-condensed table-hover" title="Balanceamento!">
           <tr >
             ...
             <th colspan=""><a href="imprimirsf-bm-excel.php" id="jn"><i class="ion-help"></i> <button type="button" class="btn btn-primary">Gerar Excel</button></a></li></th>
        ...
        while($rowjn = mysqli_fetch_assoc($lojasresult)) {
          $rowsf = $rowjn;
          $rowbm = $rowjn;
          if ($rowsf['sfestaual'] > $rowsf['sfestminino']  and $rowbm['bmestaual'] < $rowbm['bmestminino'] ){
             if($rowjn['jnestaual'] - $rowjn['jnestminino'] <  $rowbm['bmestminino'] - $rowbm['bmestaual'] and $rowjn['jnestminino'] - $rowjn['jnestaual'] < $rowsf['sfestaual'] - $rowsf['sfestminino']    ){
                #continuar aqui
             ...
             #CONDIÇÃO PARA SABER SE LOJA DE BM PRECISA DE ALO DE SF

             #CONDIÇÃO PARA SABER SE LOJA DE JN E BM PRECISA DE SF

             if ($rowbm['bmestaual'] < $rowbm['bmestminino'] and $rowsf['sfestaual'] > $rowsf['sfestminino']){
             if($difjn <=0 and $difbm < 0 and $difsf > 0){
               ...
               if(-$difjn < $difsf) {
                  if(-$difbm >= $difsf+$difjn){
                     echo "<td>" .($difsf+$difjn);"</td>";
             ...
             # FIM DA CONDIÇÃO PARA SABER SE LOJA DE JN E BM PRECISA DE SF

             #CONDIÇÃO PARA SABER SE LOJA DE BM PRECISA DE SF
              if($difjn > 0 and $difbm < 0 and $difsf > 0){
                ...
                if ($difjn < $rowbm['bmestminino'] - $rowbm['bmestaual']){
                    echo "<td>" .($rowbm['bmestminino'] -$rowbm['bmestaual']-$difjn);"</td>";
...
function CriaPDF() {
    ...
    win.document.write('<title>Balanceamento São Francisco para Brasília de Minas</title>');   // <title> CABEÇALHO DO PDF.
